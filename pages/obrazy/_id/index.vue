<template>
    <v-container fluid>
        <v-row>
            ...
            >
                ...
                <v-stepper class="mt-10" alt-labels :value="step">
                    <v-stepper-header>
                        <v-stepper-step class="pa-3" step="1" :complete="step > 1" @click="step = 1">
                            Edytuj dane
                        </v-stepper-step>

                        <v-divider></v-divider>

                        <v-stepper-step class="pa-3" step="2" :complete="step > 2" @click="step = 2">
                            Edytuj zdjęcia
                        </v-stepper-step>
        ...
        <v-form ref="form" v-if="step === 1">
            ...
            <v-row>
                <v-col :cols="$vuetify.breakpoint.mobile ? 12: 6" class="mx-auto" :class="{'my-5': $vuetify.breakpoint.mobile}">
                    <v-btn block color="green" class="white--text" @click="sendData">
                        Zapisz zmiany
                    </v-btn> 
                </v-col>
            </v-row>    
        ...
        <div v-if="step === 2">
            ...
            <v-row>
                <v-col :cols="$vuetify.breakpoint.mobile ? 12: 6" class="mx-auto">
                    <v-btn id="file" block color="green" class="white--text" @click="sendFiles">
                        Zapisz
                    </v-btn> 
                    <input style="display: none" type="file" id="file" ref="file" @change="handleFileUpload"/>
                </v-col>
...
import {DATA_MISSING, ARE_U_SURE} from '~/constants/messages';
    ...
    export default {
        ...
        async asyncData({ params, app}) {
            const { data } = await app.$repositories.painting.getById(params.id);
            const painting = new DetailedPainting(data);
            return { painting }
        },
        methods: {
            sendData() {
                if(this.$refs.form.validate()){
                    ...
                    this.$store.dispatch('paintings/update', this.painting);
                }

            },
            sendFiles() {
                    let imageFiles = this.images.filter(i => i != undefined);
                    ...
                    if(imageFiles != undefined && imageFiles.length > 0){
                        ...
                        this.step = 3;
                        this.$store.dispatch('images/create', {id: this.painting.id, formData})
                            .then(() => {
                                this.success = true;
                            })
                            .finally(() => {
            ...
            handleFileUpload(e) {
                ...
                fileReader.addEventListener('load', () => {
                    ...
                    this.images[this.imagesUrl.length - 1] = files[0];
                });
            },
            removeImage(index) {
                this.remove = index;
            ...
            handleRemoveImage() {
                ...
                let image = this.imagesUrl.splice(this.remove,1) + '';
                image = image.replace(/^.*[\\\/]/, '');
                this.$store.dispatch('images/delete', {id: this.painting.id, fileName: image});
            }
        },
        mounted() {
            this.imagesUrl = this.painting.images.map(i => i.small);
        },
        computed: {
            ...mapGetters('tags', [
                'tags'
